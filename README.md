 # LangGraph Agentic RAG 测试助手
  ...
  循环）、多轮对话记忆持久化、JWT 鉴权、对话落 MySQL、Redis 限流、Docker 容器化部署与流式输出，并封装了一层 **FastAPI
  服务 + 前端演示页**，可直接在浏览器里对话。
  ...
  按 `thread_id` 查询历史。
  - **Redis 限流（V2.0）**：`/chat/stream`、`/chat/once`、`/cases/generate` 按 `user.id` 做固定窗口限流（Redis
  `INCR`+`EXPIRE`，默认 10 次/分钟），超限返回 429，保护 LLM 接口不被刷爆。
  - **Docker 容器化（V2.0）**：`Dockerfile` + `.dockerignore` + `docker-compose.yml` 一键编排 app + MySQL 8.0 + Redis
  7，密钥经 `.env` 运行时注入、不进镜像层。
  - **统一响应格式**：所有接口返回 `{code, message, data}`，并配置全局异常处理器兜底。
  - **分层架构**：`routers/`（路由）、`models/`（表）、`schemas/`（Pydantic）、`services/`（业务）、`core/`（配置/安全/
  依赖）。
  - **测试管理平台（V1.8）**：项目/用例 CRUD、AI 生成用例（结构化输出）、执行记录、统计看板，所有数据按 `user.id`
  多租户隔离。
  ...
  │   │   └── state.py        #   状态定义参考
  │   ├── core/               # 配置、安全、依赖
  │   │   ├── config.py       #   集中读 .env（DEEPSEEK/JWT/DB/Redis/RAG 阈值/HF 镜像）
  │   │   ├── security.py     #   bcrypt + PyJWT（hash/verify/token）
  │   │   ├── deps.py         #   get_current_user 依赖
  │   │   ├── redis_client.py #   Redis 异步客户端 redis.asyncio（V2.0）
  │   │   └── rate_limit.py   #   限流依赖：固定窗口 INCR+EXPIRE（V2.0）
  ...
  ├── .env.example            # 环境变量模板
  ├── Dockerfile              # 镜像构建定义（V2.0）
  ├── .dockerignore           # 构建排除 .env/chroma_db/checkpoints.db 等（防密钥进镜像）
  └── docker-compose.yml      # app + MySQL 8.0 + Redis 7 编排（V2.0）
  ```
  ...
  2. 在项目根目录创建 `.env`（参考 `.env.example`），关键变量：
     ...
     >
     > 连接 MySQL 8.0 需要 `cryptography`（默认 `caching_sha2_password` 认证），已加入 requirements.txt；若本地 MySQL
  用的是 `mysql_native_password` 则此依赖不会触发。
  ...
  3. 构建知识库（首次运行或更新语料后执行）：
     ...
     > Embedding 走 HuggingFace 本地模型（`BAAI/bge-small-zh-v1.5`），镜像 `https://hf-mirror.com` 已集中在
  `app/core/config.py`。
  ...
  **Docker 部署（V2.0）**：
  ...
  一键启动 app + MySQL 8.0 + Redis 7 三个服务。注意：容器内通过服务名互联，Docker 环境下 `.env` 的 `DB_URL` /
  `REDIS_URL` 的 host 要写成 `mysql` / `redis`（本地开发则写 `localhost`），或由 compose 的 `environment` 覆盖。

  ## API 接口

  统一响应格式：`{"code": 0, "message": "success", "data": {...}}`（`code=0` 成功，非 0 失败）。

  > ⚠️ `/chat/stream`、`/chat/once`、`/cases/generate` 带限流（按 `user.id` 固定窗口，默认 10 次/分钟，超限返回 429）。
  ...
  - **路由分层**：`main.py` 只做 app 创建 + `include_router` 挂载，各模块路由独立在 `routers/`，用 `APIRouter`
  避免循环导入。
  - **防幻觉两层过滤**：向量分数粗筛（L2 阈值）+ 系统提示强制"先 `search_kb` 再回答，禁止凭记忆编造"。
  - **Redis 限流（固定窗口）**：`make_rate_limit` 工厂按接口生成不同限额的依赖，Redis `INCR` 原子自增计数，`if
  current==1` 才设 `EXPIRE`（避免每次请求重置窗口的竞态），超限抛 429；`scope` 前缀区分不同接口计数。
  ...
  - 401/429 返回的是 FastAPI 默认 `{"detail": ...}`，未统一进 `{code, message, data}`。
  - 限流目前是固定窗口，可升级滑动窗口 / 令牌桶。
  - 部署云服务器 + README 在线 demo（V2.0 剩余项）。